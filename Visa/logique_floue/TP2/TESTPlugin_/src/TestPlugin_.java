...
public class TestPlugin_ implements PlugInFilter, DialogListener {
	...
	public void run(ImageProcessor ip) {
		...
		ImageProcessor resultImage = fpLaplacian;
		...
		if (seuillageZeroCross) {
			resultImage = laplacienZero(fpLaplacian, seuilZeroCross);
		}
		
		ImagePlus imageResult = new ImagePlus("Resultat", resultImage);
		imageResult.show();
	}
				...
				for (int vy = -1; vy < 1; ++vy) {
					for (int vx = -1; vx < 1; ++vx) {
						...
						if (px >= 0 && px < width && py >= 0 && py < height) {
							max = Math.max(max, imLaplacien.getf(px, py));
							min = Math.min(min, imLaplacien.getf(px, py));
						}
						
					}
				}
				imZeros.putPixel(x, y, (max > seuil && min < -seuil) ? 255 : 0);
			}
		}
